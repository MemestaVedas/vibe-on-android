...
class PlaybackStatsCalculator {
    ...
    fun loadSummary(
        ...
        val (startBound, endBound) = range.resolveBounds(events, nowMillis, zoneId)
        val filteredEvents = events.mapNotNull { event ->
            val start = event.startMillis()
            val end = event.endMillis()
            val lowerBound = startBound ?: Long.MIN_VALUE
            if (end < lowerBound || start > endBound) return@mapNotNull null
            ...
            if (effectiveDuration <= 0L) return@mapNotNull null
        ...
        val effectiveStart = startBound
            ...
            ?: events.minOfOrNull { it.startMillis() }
        ...
        val allSongs = segmentsBySong
            .mapNotNull { (songId, segmentsForSong) ->
                ...
                val title = song.title.takeIf { it.isNotBlank() }
                    ?: song.id.substringAfterLast('/').ifBlank { return@mapNotNull null }
        ...
        val durationsByDayOfWeek = daySlices.groupBy { slice -> slice.date.dayOfWeek }
    ...
    private fun normalizeEventDuration(
        ...
        val trackDuration = song?.duration?.takeIf { it > 0 }?.let { (it * 1000L).toLong() }
    ...
    private fun createMonthBuckets(zoneId: ZoneId, now: Instant): List<TimelineBucket> {
        ...
        return buildList {
            repeat(bucketCount) { index ->
                ...
                if (startDay > daysInMonth) return@repeat
